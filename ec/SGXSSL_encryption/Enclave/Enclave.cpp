...
void train_logistic_regression1(unsigned char* encrypted_data, float lr, int iter, unsigned int data_rows, unsigned int data_cols, unsigned int data_len, float* weights)
{
	const unsigned int data_elems = data_cols * data_rows;
	unsigned char * decrypted = (unsigned char *)malloc(sizeof(float)*data_elems + data_rows);

	RSA_private_decrypt(data_len, encrypted_data, decrypted, context, RSA_PKCS1_PADDING);

	float * data = (float*)decrypted;
	char * labels = (char*)(decrypted + sizeof(float)*data_elems);
