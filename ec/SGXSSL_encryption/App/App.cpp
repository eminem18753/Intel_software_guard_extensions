...
float calculatePredictionError(float *data,float *weights,int *groundTruth,int rowNumber,int colNumber)
{
    float accuracy=0;
...
int main(int argc, char const *argv[]) 
{
    ...
    public_key=(unsigned char*)malloc(sizeof(char)*(keyLength+1));
    get_public_key(global_eid,public_key,keyLength);
    cout<<"Length:"<<keyLength+1<<endl;
    ...
    */
	...
	//{
		cout<<"inside1"<<endl;
		valwrite=send(new_socket,public_key,keyLength,0);
