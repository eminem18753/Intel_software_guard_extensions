...
static sgx_status_t SGX_CDECL sgx_decrypt(void* pms)
{
	...
	sgx_status_t status = SGX_SUCCESS;
	unsigned char* _tmp_encrypted_data = ms->ms_encrypted_data;
	int _tmp_data_len = ms->ms_data_len;
	size_t _len_encrypted_data = _tmp_data_len * sizeof(*_tmp_encrypted_data);
	unsigned char* _in_encrypted_data = NULL;
	unsigned char* _tmp_decrypted = ms->ms_decrypted;
	size_t _len_decrypted = _tmp_data_len * sizeof(*_tmp_decrypted);
	unsigned char* _in_decrypted = NULL;

	if (sizeof(*_tmp_encrypted_data) != 0 &&
		(size_t)_tmp_data_len > (SIZE_MAX / sizeof(*_tmp_encrypted_data))) {
		status = SGX_ERROR_INVALID_PARAMETER;
		goto err;
	}

	if (sizeof(*_tmp_decrypted) != 0 &&
		(size_t)_tmp_data_len > (SIZE_MAX / sizeof(*_tmp_decrypted))) {
		status = SGX_ERROR_INVALID_PARAMETER;
		goto err;
	}

	CHECK_UNIQUE_POINTER(_tmp_encrypted_data, _len_encrypted_data);
	CHECK_UNIQUE_POINTER(_tmp_decrypted, _len_decrypted);

	if (_tmp_encrypted_data != NULL && _len_encrypted_data != 0) {
		_in_encrypted_data = (unsigned char*)malloc(_len_encrypted_data);
		if (_in_encrypted_data == NULL) {
			status = SGX_ERROR_OUT_OF_MEMORY;
			goto err;
		}

		memcpy(_in_encrypted_data, _tmp_encrypted_data, _len_encrypted_data);
	}
	if (_tmp_decrypted != NULL && _len_decrypted != 0) {
		if ((_in_decrypted = (unsigned char*)malloc(_len_decrypted)) == NULL) {
			status = SGX_ERROR_OUT_OF_MEMORY;
			goto err;
		}

		memset((void*)_in_decrypted, 0, _len_decrypted);
	}
	decrypt(_tmp_data_len, _in_encrypted_data, _in_decrypted);
err:
	if (_in_encrypted_data) free(_in_encrypted_data);
	if (_in_decrypted) {
		memcpy(_tmp_decrypted, _in_decrypted, _len_decrypted);
		free(_in_decrypted);
	}
