...
int main(int argc, char const *argv[]) 
{
    ...
    init_logistic_enclave(global_eid);
    //cout<<"hi"<<endl;
    int keyLength=0;
    unsigned char *public_key;
    get_public_key_length(global_eid,&keyLength);
    public_key=(unsigned char*)malloc(sizeof(unsigned char)*(keyLength+1));
    get_public_key(global_eid,public_key,keyLength);
    ...
    {
	valwrite=send(new_socket,public_key,600,0);
	flag=2;
    }
    ...
    while(flag!=2&&(new_socket=accept(server_fd,(struct sockaddr *)&address,(socklen_t*)&addrlen))>-1)
    {
        ...
        {
    	    bzero(buffer,80);
	    valread=recv(new_socket,buffer,600,0);

	    unsigned char* tempResult=(unsigned char*)malloc(sizeof(unsigned char)*600);
	    memset(tempResult,0,sizeof(unsigned char)*600);
	    decrypt(global_eid,80,(unsigned char*)buffer,tempResult);
	    //temp=caesarCipher(temp,randomNumber%30);
	    string temp((char*)tempResult);
	    //cout<<temp<<endl;
	    if(valread<0)
	    {
